#ifndef _TI_SPRINTF_H
#define _TI_SPRINTF_H
...
({\
    ...
    if (__str_len >= 0) {\
        ...
        *__p_buffer = (char*)malloc(__buffer_size);\
        if (*__p_buffer != NULL) {\
            __ret = _ti_sprintf(*__p_buffer, __VA_ARGS__);\
        }\
    }\
    __ret;\
})
