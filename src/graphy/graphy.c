 ...
 * v6 functions
 * ------------
 * Broken   gfx_RLETSprite
 * Broken   gfx_RLETSprite_NoClip
...
/**
 * @brief examples/library_examples/graphx status
 * 
 * background           > 
 * hello_world          > 
...
static int gfy_ClipXMin = 0;
static int gfy_ClipYMin = 0;
static int gfy_ClipXMax = GFY_LCD_WIDTH;
static int gfy_ClipYMax = GFY_LCD_HEIGHT;
...
void gfy_SetPixel(uint24_t x, uint8_t y) {
    if (x < GFY_LCD_WIDTH && y < GFY_LCD_HEIGHT) {
        ((uint8_t*)gfy_CurrentBuffer)[(uint24_t)y + (x * GFY_LCD_HEIGHT)] = gfy_Color;
    }
}
...
uint8_t gfy_GetPixel(uint24_t x, uint8_t y) {
    return ((uint8_t*)gfy_CurrentBuffer)[(uint24_t)y + (x * GFY_LCD_HEIGHT)];
}
...
static void gfy_internal_PrintCharXY(const char c, int posX, int posY) {
    if (gfy_PrintChar_Clip == gfy_text_clip) {
        if (
            ...
            posX >= gfy_ClipXMax || posY >= gfy_ClipYMax ||
            posX + 8 * gfy_TextWidthScale > gfy_ClipXMax ||
            posY + 8 * gfy_TextHeightScale > gfy_ClipYMax
        ) {
            return;
        }
    }
...
static void gfy_internal_PlotCirclePoints(int x, int y, int x_pos, int y_pos) {
...
void gfy_Circle(int x, int y, uint24_t radius) {
...
void gfy_FillCircle(int x, int y, uint24_t radius) {
    // Incorrect clipping
    if (
        x - (int)radius >= gfy_ClipXMin && y - (int)radius >= gfy_ClipYMin &&
        x - (int)radius < gfy_ClipXMax && y - (int)radius >= gfy_ClipYMax
    ) {
...
void gfy_FillCircle_NoClip(uint24_t x, uint8_t y, uint24_t radius) {
    ...
    while (x_pos >= y_pos)
    {
        for (int i = (int)x - x_pos; i <= (int)x + x_pos; i++)
        {
            gfy_SetPixel(i, (int)y + y_pos);
            gfy_SetPixel(i, (int)y - y_pos);
        }
        for (int i = (int)x - y_pos; i <= (int)x + y_pos; i++)
        {
            gfy_SetPixel(i, (int)y + x_pos);
            gfy_SetPixel(i, (int)y - x_pos);
        }
...
void gfy_ShiftDown(uint8_t pixels) {
    if (pixels == 0) { return; }
    const uint8_t* src_buf = (uint8_t*)gfy_CurrentBuffer + gfy_ClipYMin + (gfy_ClipXMin * GFY_LCD_HEIGHT);
    uint8_t* dst_buf = (uint8_t*)gfy_CurrentBuffer + (gfy_ClipYMin + pixels) + (gfy_ClipXMin * GFY_LCD_HEIGHT);
...
void gfy_ShiftLeft(uint24_t pixels) {
    if (pixels == 0) { return; }
    const uint8_t* src_buf = (uint8_t*)gfy_CurrentBuffer + gfy_ClipYMin + (gfy_ClipXMin * GFY_LCD_HEIGHT);
    uint8_t* dst_buf = (uint8_t*)gfy_CurrentBuffer + gfy_ClipYMin + ((gfy_ClipXMin - pixels) * GFY_LCD_HEIGHT);
    const uint24_t copySize = gfy_ClipYMax - gfy_ClipYMin;
    int x0 = gfy_ClipXMin + pixels;
    int x1 = gfy_ClipXMax;
...
void gfy_Sprite(const gfy_sprite_t *sprite, int x, int y) {
    ...
    if (
        min_clipX + max_clipX >= sprite->width ||
        min_clipY + max_clipY >= sprite->height
    ) {
        return;
    }
    ...
    uint8_t* dst_buf = (uint8_t*)gfy_CurrentBuffer + (y + min_clipY) + ((x + min_clipX) * GFY_LCD_HEIGHT);
...
void gfy_ScaledSprite_NoClip(
    ...
    // memcpy columns
    const uint8_t* src_col = (uint8_t*)gfy_CurrentBuffer + y + (x * GFY_LCD_HEIGHT);
...
void gfy_RLETSprite(const gfy_rletsprite_t *sprite, int x, int y) {
    gfx_RLETSprite((const gfx_rletsprite_t*)sprite, x, y);
}

/* gfy_RLETSprite_NoClip */

void gfy_RLETSprite_NoClip(const gfy_rletsprite_t *sprite, uint24_t x, uint8_t y) {
    gfx_RLETSprite_NoClip((const gfx_rletsprite_t*)sprite, x, y);
}
