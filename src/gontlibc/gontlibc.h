...
} gontlib_load_options_t;

/**
 * Options for selecting a font by style
 * @see gontlib_GetFontByStyle
 */
typedef enum {
    /**
 ...
 * @see gontlib_font_pack_t
 */
typedef struct gontlib_metadata_t {
    /**
 ...
 *  gontlib_font_t *my_font = gontlib_GetFontByStyle("FONTPACK", 12, 12,
 *      FONTLIB_NORMAL, FONTLIB_NORMAL, FONTLIB_SERIF, 0);
 *  if (!my_font || !gontlib_SetFont(my_font))
 *      return;
...
typedef struct gontlib_font_t {
     ...
     * @see gontlib_weights_t
     */
    uint8_t weight;
    /**
     * Specifies the style of the font.
     * @see gontlib_styles_t
     */
    uint8_t style;
    /**
 ...
 * @code{.cpp}
 *  FILE font_pack_file;
 *  gontlib_font_pack_t *font_pack;
 *  gontlib_metadata_t *metadata;
 *  char *var_name;
 *  char *font_author;
 ...
 *  if (metadata->length < sizeof(gontlib_metadata_t))
 *      return;
 *  // Get font's author
 ...
 * @endcode
 */
typedef struct gontlib_font_pack_t {
    /**
     * Must be "FONTPACK"
 ...
 * This is the default.
 */
void gontlib_SetWindowFullScreen(void);
 ...
 */
void gontlib_SetWindow(unsigned int x_min, uint8_t y_min, unsigned int width, uint8_t height);
 ...
 * @param[in] y Y
 */
void gontlib_SetCursorPosition(unsigned int x, uint8_t y);
 ...
 * @param[in] y y-shift
 */
void gontlib_ShiftCursorPosition(int x, int y);
 ...
 */
bool gontlib_SetFont(const gontlib_font_t *font_data, gontlib_load_options_t flags);
 ...
 */
void gontlib_SetForegroundColor(uint8_t color);
 ...
 */
void gontlib_SetBackgroundColor(uint8_t color);

/**
 * Sets the current colors FontLibC will use for drawing.
 * @note These are NOT the same as GraphY's current colors!
 * @param[in] forecolor New foreground color to use
 * @param[in] backcolor New background color to use
 */
void gontlib_SetColors(uint8_t forecolor, uint8_t backcolor);
 ...
 */
void gontlib_SetTransparency(bool transparency);
 ...
 */
void gontlib_SetLineSpacing(uint8_t space_above, uint8_t space_below);
 ...
 */
void gontlib_SetItalicSpacingAdjustment(uint8_t italic_spacing_adjustment);
 ...
 */
void gontlib_SetNewlineCode(char code_point);
 ...
 */
void gontlib_SetAlternateStopCode(char code_point);
 ...
 */
void gontlib_SetFirstPrintableCodePoint(char code_point);
 ...
 */
void gontlib_SetDrawIntCodePoints(char minus, char zero);
 ...
 * gontlib_SetFirstPrintableCodePoint().  It can even draw code point 0.
 ...
 * the character code specified with gontlib_SetAlternateStopCode().
 ...
 * directly call gontlib_DrawGlyph to force display the character and increment
 * past it.
 * @note Newline codes will print regardless of whether FONTLIB_ENABLE_AUTO_WRAP
 * is enabled.  To disable parsing newline codes, use gontlib_SetNewlineCode(0);
 ...
 */
void gontlib_DrawInt(int n, uint8_t length);
 ...
 * @note \c length must be between 1 and 8, inclusive
 */
void gontlib_DrawUInt(unsigned int n, uint8_t length);
 ...
 */
void gontlib_ClearEOL(void);

/**
 * Erases the entire current text window.
 * @note This ignores the transparent background flag.  If background
 * transparency is important, check gontlib_GetTransparency() before calling
 * this routine.
 */
void gontlib_ClearWindow(void);
 ...
 */
bool gontlib_Newline(void);

/**
 * Sets options for controlling newline behavior
 * @param[in] options Flags setting newline behavior
 * @see gontlib_newline_options_t
 */
void gontlib_SetNewlineOptions(uint8_t options);
 ...
 */
char *gontlib_GetFontPackName(const char *appvar_name);
 ...
 * @return Direct pointer to font, or NULL if the index is invalid.
 */
gontlib_font_t *gontlib_GetFontByIndex(const char *font_pack_name, uint8_t index);
